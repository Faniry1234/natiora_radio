...
class Emissions {
    ...
    public function getAll(){
        ...
        if (!$this->db) {
            // PHP fallback file (only PHP, no JSON)
        ...
        // Use CASE for ordering to be compatible with SQLite (no FIELD() function)
        $stmt = $this->db->prepare("SELECT * FROM emissions ORDER BY CASE day
            WHEN 'lundi' THEN 1
            WHEN 'mardi' THEN 2
    ...
    public function getByDay($day){
        ...
        $stmt = $this->db->prepare("SELECT * FROM emissions WHERE day = :day ORDER BY time");
    ...
    public function add($day, $emission){
        ...
        $stmt = $this->db->prepare("INSERT INTO emissions (day, time, title, presenter, duration, level, category, src, description) 
                                   VALUES (:day, :time, :title, :presenter, :duration, :level, :category, :src, :description)");
        return $stmt->execute([
            ':day' => $day,
    ...
    public function update($day, $index, $emission){
        ...
        $stmt = $this->db->prepare("UPDATE emissions SET time = :time, title = :title, presenter = :presenter, 
                                   duration = :duration, level = :level, category = :category, src = :src, description = :description 
                                   WHERE id = :id");
        return $stmt->execute([
            ':id' => $id,
            ':time' => $emission['time'] ?? null,
    ...
    public function delete($day, $index){
        ...
        $id = $emissions[$index]['id'];
        $stmt = $this->db->prepare("DELETE FROM emissions WHERE id = :id");
